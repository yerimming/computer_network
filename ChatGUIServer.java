...
class ServerThread extends Thread {
    ...
    public void run() {
        try {
            //승인 여부
            int var = JOptionPane.showConfirmDialog(null, "승인하시겠습니까?", "승인", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE, null);
            ...
            String[] ntn = br.readLine().split(",");
            name = ntn[0];//자기 이름
            toname = ntn[1];//원하는 대상 이름
            if(toname==null)
            {
                new Thread(new ChatGUIClient1(name)).start();
            }
            /*
            try {

                String sql_query = String.format("select id from user where id = '%s';",toname);
    ...
    public Connection getConnection() throws SQLException {
        ...
        conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/network", "root",
                "Namja241377@");
